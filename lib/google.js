    ...
    if ((err == null) && resp.statusCode === 200) {
      ...
      $(itemSel).each(function (i, elem) {
        ...
        if(item.ratingfull.length > 0)
        {
          ...
          //RATING
          var regex_rating = /(Rating:|Bewertung:) ([^\s]+)/;
          var match_rating = item.ratingfull.match(regex_rating);
          if(match_rating)
          item.rating = match_rating[2];
          ...
          //REVIEWS
          var regex_reviews = /([^\s]+) (reviews|votes|Rezensionen|Ergebnisse)/;
          ...
          //PRICE
          var regex_price = /(Price range:|Preisspanne:) ([^.?]+)/;
          var match_price = item.ratingfull.match(regex_price);
          if(match_price)
          item.price = match_price[2].replace(' ','');
