...
var igoogle = function (query, start, callback) {
  ...
  request(requestOptions, function (err, resp, body) {
    if ((err == null) && resp.statusCode === 200) {
      ...
      //ADD GOOGLE BLOCK
      var grating = $(googleBoxSel).find("g-review-stars").parent().find("span").first().text(); //e.g. 3,9  //$(googleBoxSel).find(".ul7Gbc").parent().children().first().text()
      var gratingcount = $(googleBoxSel).find("g-review-stars").parent().find("a").first().text()
