...
var igoogle = function (query, start, callback) {
  ...
  request(requestOptions, function (err, resp, body) {
    if ((err == null) && resp.statusCode === 200) {
      ...
      //ADD GOOGLE BLOCK
      var grating = $(googleBoxSel).find(".rtng").first().text(); //e.g. 3,9  //$(googleBoxSel).find(".ul7Gbc").parent().children().first().text()
      var gratingcount = $(googleBoxSel).find(".rtng").first().parent().children().last().text()
      ...
      if(grating != "")
      {
        var item = {
          title: "GOOGLE",
          link: "https://google.com"+$(googleBoxSel).find(".thumb a").attr("href"),
          description: $(googleBoxSel).find(".ggV7z").text(), //$(googleBoxSel).find("._tA").first().text(),
          href: "https://google.com"+$(googleBoxSel).find(".thumb a").attr("href"),  //"https://google.com",
